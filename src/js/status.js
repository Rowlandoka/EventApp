...
class Status {
	...
	loadData() {
		apiCall('statistics')
			.then((response) => {
				...
				this.$tabArea.classList.remove('invisible');
				this.$chartArea.classList.remove('invisible');
	...
	hideCharts() {
		...
		this.$ageCanvas.classList.add('invisible');
		this.$professionCanvas.classList.add('invisible');
		this.$experienceCanvas.classList.add('invisible');
	}
	...
	loadExperience(event = null) {
		...
		this.hideCharts();
		this.$experienceCanvas.classList.remove('invisible');
	...
	loadProfession(event = null) {
		if (event) event.preventDefault();
		this.hideCharts();
		this.$professionCanvas.classList.remove('invisible');
	...
	loadAge(event = null) {
		if (event) event.preventDefault();
		this.hideCharts();
		this.$ageCanvas.classList.remove('invisible');
		...
		const data = {
			...
			labels: ['10-15 years', '15-20 years', '20-25 years'],
		};
